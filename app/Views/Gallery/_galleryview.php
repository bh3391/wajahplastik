...
<div class="container md:mt-10 mx-auto">
    ...
    <form action="gallery/search" method="post" class="w-full p-2 " x-data="{search : ' ' }">
        ...
        <?php if (session()->getFlashdata('Berhasil')) : ?>
            <div class="alert text-white  p-2 bg-green-500 rounded-sm shadow text-sm text-left"><i class="fas fa-check"></i><?= session()->getFlashdata('Berhasil') ?></div>
        ...
        <?php if (session()->getFlashdata('Gagal')) : ?>
            <div class="alert text-white  p-2 bg-red-400 rounded-lg shadow text-sm text-left"><i class="fas fa-trash-alt p-2"></i><?= session()->getFlashdata('Gagal') ?></div>
        <?php endif; ?>
